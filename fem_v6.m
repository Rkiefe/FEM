...
%% Fields
% clearvars -except Bx By mu t p xs ys nt nv pgon

[M,H] = magn(nt,Bx,By,mu,t,xs,ys);
...
function [M,H] = magn(nt,Bx,By,mu,t,xs,ys)
mu0 = pi*4e-7;  % permiability in vacum
mut = zeros(nt,1);
for it = 1:nt
    mut(it) = sum(mu(t(:,it)))/3;
end
...
figure
subplot(1,3,1)
Bplt = quiver(xs,ys,Bx,By);
title('B')
Bplt.Parent.XLim = [35 65];
Bplt.Parent.YLim = [10 40];

subplot(1,3,2)
Hplt = quiver(xs,ys,Hx,Hy);
title('H')
Hplt.Parent.XLim = [35 65];
Hplt.Parent.YLim = [10 40];

subplot(1,3,3)
Mplt = quiver(xs,ys,Mx,My);
title('M')

Mplt.Parent.XLim = [35 65];
Mplt.Parent.YLim = [10 40];
...
Mint = sqrt(Mx.^2 + My.^2);
% plot3(xs,ys,Mint,'.')
scatter(xs,ys,10,Mint.*mu0)
cb = colorbar;
cb.Label.String = {'Magnetization (T)'};
